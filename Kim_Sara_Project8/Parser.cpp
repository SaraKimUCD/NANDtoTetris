...
VM_Command Parser::commandType() {
    ...
    string fiveLetters = command.substr(0, 5);
    string sixLetters = command.substr(0, 6);
    string eightLetters = command.substr(0, 8);
    ...
    // Project 8: C_LABEL, C_GOTO, C_IF, C_FUNCTION, C_RETURN, C_CALL
    else if(fiveLetters == "label"){
        return C_LABEL;
    }
    else if(fourLetters == "goto"){
        return C_GOTO;
    }
    else if(twoLetters == "if"){
        return C_IF;
    }
    else if(eightLetters == "function"){
        return C_FUNCTION;
    }
    else if(sixLetters == "return"){
        return C_RETURN;
    }
    else if(fourLetters == "call"){
        return C_CALL;
    }
 ...
 * Called only if the current command is
...
int Parser::arg2() {
    if(commandType() == C_PUSH || commandType() == C_POP){
        ...
        // need to convert the found string/arg2 into integer to return
        intTemp = stoi(temp);
        return intTemp;
    }
    return 0;
